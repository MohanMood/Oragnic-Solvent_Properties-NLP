...
df = pd.read_csv(input_dir + 'Organic-Solvents_Viscosity-Sigma_Features.csv', encoding='unicode_escape')
...
standard_columns_sigma = ['S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'S7', 'S8', 'S9', 'S10']
standard_columns_True = ['Temperature (K)']

# Split the data into features and target (adjust 'target_column' accordingly)
X = dataset[standard_columns_sigma + standard_columns_True]
y = dataset['log_visc']
mol_smiles = dataset['Canonical_SMILES']
temperature = dataset['Temperature (K)']

# Create transformers for each type of scaling
scaler_SFS_sigma = StandardFlexibleScaler(column_wise=False)
scaler_SFS_True = StandardFlexibleScaler(column_wise=True)

# Create the ColumnTransformer
preprocessor = ColumnTransformer(
    transformers=[
        ('standard_scaler_sigma', scaler_SFS_sigma, standard_columns_sigma),
        ('standard_scaler', scaler_SFS_True, standard_columns_True)], remainder='passthrough')
...
columns = standard_columns_sigma + standard_columns_True
...
# Save to CSV
combined_df.to_csv('CATBoost_Sigma-Profile-Viscosity-Quota.csv', index=False)

...
df = pd.read_csv(input_dir + 'VOC-Database.csv')

# Sorting dataset with unique SMILES
df = df.drop_duplicates(subset=['SMILES']).sort_values(by='SMILES')
print("unique_mol_smiles shape:", df.shape)
...
# Define the percentage for initial sampling (80%)
percentage = 80

# Get the sampled indexes (80% of data)
...
# Label Quota samples (80% of data)
tsne_df.loc[strat_samp, 'Label'] = label_2

# Extract Quota subset (80% of data)
dataset = pd.read_csv(input_dir + 'Oragnic-Solvents_enthalpy_Canonical_SMILES.csv', encoding='unicode_escape')
...
fps_quota = binary_fps(smiles_sampling, fp_type='ECFP4', n_bits=256)
np.save(input_dir + 'Oragnic-Solvents_enthalpy_MFs_Sampling.npy', fps_quota)
fingerprints_sampling = np.load(input_dir + 'Oragnic-Solvents_enthalpy_MFs_Sampling.npy')

# Further split data into 70% training and 10% validation
...
cat_model = CatBoostRegressor(verbose = 100, n_estimators = 1864, l2_leaf_reg = 1, learning_rate = 0.03346960601160188, boosting_type = 'Plain', depth = 7)
...
train_df = pd.DataFrame({
    'SMILES': smiles_train,
    'enthalpy_exp': y_train,
    'enthalpy_pred': y_pred_train,
    'Set': 'Training'
})
...
# Save to CSV
combined_df.to_csv('CATBoost_Mol2vec-enthalpy.csv', index=False)